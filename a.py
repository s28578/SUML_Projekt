...
if selected_option == "Strona Główna":
    ...
    with tab3:
        ...
        tabb1, tabb2, tabb3, tabb4, tabb5, tabb6, tabb7, tabb8 = st.tabs(["Płeć", "Długość",
                                          "Średnica", "Wysokość", "Całkowita waga", "Waga po wyłuskaniu",
                              "Waga trzewi", "Waga muszli"])
        with tabb1:
            st.write("Płeć")
            with st.container(border=True):
                st.image("rozklad_plec.png")
        with tabb2:

            st.write("Długość")
            with st.container(border=True):
                st.image("rozklad_dlugosc.png")
        with tabb3:

            st.write("Średnica")
            with st.container(border=True):
                st.image("rozklad_diameter.png")

        with tabb4:
            st.write("Wysokość")
            with st.container(border=True):
                st.image("rozklad_wysokosc.png")

        with tabb5:
            st.write("Całkowita waga")
            with st.container(border=True):
                st.image("rozklad_wholeweight.png")

        with tabb6:
            st.write("Waga po wyłuskaniu")
            with st.container(border=True):
                st.image("rozklad_shuckedweight.png")
        with tabb7:
            st.write("Waga trzewi")
            with st.container(border=True):
                st.image("rozklad_visceraweight.png")
        with tabb8:
            st.write("Waga muszli")
            with st.container(border=True):
                st.image("rozklad_shellweight.png")
